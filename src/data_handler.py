...
# def load_and_validate_data() -> Dict[str, Any]:
#     """Database version that matches the original JSON structure"""
#     from app import (  # Local import to avoid circular dependencies
#         Job, Technician, Equipment, Tool, Material
#     )
#     from sqlalchemy.orm import joinedload
...
#             'required_tools': [{'tool_id': tool.tool_id, 'quantity': tool.quantity} for tool in job.tools],
#             'required_materials': [{'material_id': mat.material_id, 'quantity': mat.quantity} for mat in job.materials],
#             'precedence': [precedes.precedes_job_id for precedes in job.preceded_by]
#         } for job in Job.query.options(
#             joinedload(Job.skills),
#             joinedload(Job.tools),
#             joinedload(Job.materials),
#             joinedload(Job.preceded_by)
#         ).all()],
        
#         'technicians': [{
#             'tech_id': tech.tech_id,
#             'name': tech.name,
#             'skills': [skill.skill for skill in tech.skills],
#             'hourly_rate': tech.hourly_rate,
#             'email': tech.email
#         } for tech in Technician.query.options(joinedload(Technician.skills)).all()],
        
#         'equipment': [{'equipment_id': e.equipment_id} for e in Equipment.query.all()],
#         'tools': [{'tool_id': t.tool_id, 'quantity': t.quantity} for t in Tool.query.all()],
#         'materials': [{'material_id': m.material_id, 'quantity': m.quantity} for m in Material.query.all()],
#         't_start': '2025-07-01T08:00:00',
#         't_end': '2025-07-07T18:00:00'
#     }
